...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if username == tl_admin_user or user_info:  # validate user
            ...
            bot.sendMessage(5061352415,intento_msg)
            return
        ...
        if '/account' in msgText:
            try:
                ...
                if getUser:
                    ...
                    jdb.save()
                    statInfo = infos.createStat(username,getUser,jdb.is_admin(username))
                    bot.sendMessage(update.message.chat.id,'Configuración añadida')
            except:
        ...
        if '/tokenize_on' in msgText:
            try:
                ...
                if getUser:
                    ...
                    jdb.save()
                    statInfo = infos.createStat(username,getUser,jdb.is_admin(username))
                    bot.sendMessage(update.message.chat.id,'Porfavor vuelva a desactivar esta opcion')
            except:
...
def main():
    ...
    #set in debug
    bot_token = '5347610697:AAEHt62no3ioub7ghin5SWZae5qi6LpAfxk'
